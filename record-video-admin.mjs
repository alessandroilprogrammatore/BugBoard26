...
const OUTPUT_DIR = path.join(__dirname, 'Documentazione', 'Documentazione finale', 'immagini', 'frames_admin');
...
(async () => {
    ...
    await moveAndClickUrl('/admin/users');
    await sleep(3500);

    // Report
    console.log('Going to reports');
    await moveAndClickUrl('/reports');
    await sleep(3500);

    // Archivio
    console.log('Going to archive');
    await moveAndClickUrl('/archive');
    await sleep(3000);

    // Export
    console.log('Going to export');
    await moveAndClickUrl('/export');
    await sleep(3000);

    // Bugs and Assign
    console.log('Going to bugs for assignment');
    ...
    try {
        ...
        if (foundBugHref) {
            ...
            await moveAndClickUrl(foundBugHref + '/assign');
            await sleep(3000);
        }
    } catch (e) { console.log(e); }
    ...
    try {
        const videoPath = path.join(__dirname, 'Documentazione', 'Documentazione finale', 'immagini', 'demo_admin_fluida_finale.mp4');
