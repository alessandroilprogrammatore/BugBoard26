...
\fancyhf{}
\fancyhead[L]{\thepage}
\fancyhead[R]{BugBoard26 --- Specifica dei Requisiti Software}
...
\centering
\vspace*{2cm}
{\LARGE\bfseries Universit\`a degli Studi di Napoli Federico II\par}
\vspace{0.3cm}
{\large Corso di Ingegneria del Software --- Canale 2\par}
\vspace{3cm}
\rule{\textwidth}{0.8pt}
\vspace{0.5cm}
{\Huge\bfseries BugBoard26\par}
\vspace{0.3cm}
{\Large Documento di Specifica dei Requisiti Software (SRS)\par}
\vspace{0.5cm}
\rule{\textwidth}{0.8pt}
\vspace{2cm}
...
\item[\textbf{Admin}] Utente con privilegi amministrativi completi. Pu\`o gestire utenti, accedere a tutti i bug del sistema, generare report e modificare configurazioni.
...
\item[\textbf{Bug}] Segnalazione di un comportamento anomalo, errato o inatteso di un sistema software. Nel contesto di BugBoard26 il termine indica genericamente qualsiasi segnalazione inserita nel sistema, indipendentemente dal tipo (Bug, Feature, Enhancement).
...
\item[\textbf{Enhancement}] Tipo di segnalazione che indica una richiesta di miglioramento di una funzionalit\`a gi\`a esistente. Distinto da un Bug (errore) e da una Feature (nuova funzionalit\`a).
...
\item[\textbf{Priorit\`a}] Livello di urgenza di un bug. I valori previsti sono: \emph{Low}, \emph{Medium}, \emph{High}, \emph{Critical}.
...
\item[\textbf{Report Mensile}] Documento generato dal sistema contenente statistiche aggregate sull'attivit\`a del periodo selezionato: numero di bug creati, risolti, tempo medio di risoluzione, distribuzione per priorit\`a e tipo.
...
\item[\textbf{Tipo (Type)}] Categoria della segnalazione. I valori previsti sono: \emph{Bug}, \emph{Feature}, \emph{Enhancement}.

\item[\textbf{USER (Utente registrato)}] Ruolo standard. L'utente pu\`o creare bug, commentare, caricare allegati, assegnare bug (se Admin) e gestire il proprio profilo.
...
\item[\textbf{Utente non registrato (Guest)}] Chiunque acceda all'applicazione senza credenziali. Pu\`o accedere alla pagina di login e di registrazione. Non ha accesso ad alcuna funzionalit\`a interna.

\item[\textbf{Utente registrato (USER)}] Utente autenticato con ruolo standard. Pu\`o creare e modificare bug, aggiungere commenti e allegati, visualizzare lo storico, gestire il proprio profilo.
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item Archiviazione automatica bug dopo periodo di inattivit\`a
...
\begin{itemize}
  \item \textbf{Registrazione al sistema tramite email}: L'utente pu\`o creare un nuovo account fornendo email, password e informazioni personali. Il sistema invia una email di conferma per verificare l'indirizzo.
...
\begin{itemize}
  ...
  \item \textbf{Creazione nuovo bug}: Inserimento di una nuova segnalazione con descrizione dettagliata, assegnazione priorit\`a, tipo e label appropriate.
  \item \textbf{Modifica bug propri o assegnati}: Possibilit\`a di aggiornare informazioni, stato e dettagli dei bug creati dall'utente o assegnati al suo account.
  ...
  \item \textbf{Assegnazione bug ad altri utenti}: Trasferimento della responsabilit\`a di un bug a un altro utente registrato nel sistema.
...
\begin{itemize}
  ...
  \item \textbf{Generazione report mensili}: Creazione di report dettagliati sull'attivit\`a del sistema per periodo specificato.
...
\begin{itemize}
  ...
  \item \textbf{Pulizia automatica dati obsoleti}: Eliminazione periodica di dati temporanei, cache e informazioni non pi\`u necessarie.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagramma_generale.png}
...
\hline
Step 5 & Seleziona la priorit\`a (Low, Medium, High, Critical) & Mostra le opzioni con colori identificativi \\
\hline
...
\cline{2-3}
 & 8.b & L'utente pu\`o eseguire una nuova ricerca \\
\hline
\end{tabular}
...
\cline{2-3}
 & 4.b & L'admin pu\`o eseguire una nuova ricerca \\
\hline
Bug gi\`a assegnato & 6.a & Chiede conferma per il cambio di assegnazione \\
\cline{2-3}
...
\hline
Preconditions & L'utente deve essere autenticato e assegnatario del bug; il bug deve essere in stato \emph{In Progress} \\
\hline
Success End Condition & Il bug viene aggiornato allo stato \emph{Resolved} con data di risoluzione registrata \\
\hline
...
\hline
Step 2 & Clicca su ``Risolvi Bug'' & Verifica che il bug sia in stato \emph{In Progress} \\
\hline
Step 3 & & Mostra il form di risoluzione \\
\hline
Step 4 & Seleziona il tipo di risoluzione & Mostra opzioni: \emph{Fixed}, \emph{Won't Fix}, \emph{Duplicate}, \emph{Cannot Reproduce} \\
\hline
...
\hline
Step 8 & & Aggiorna lo stato a \emph{Resolved} \\
\hline
...
\textbf{EXTENSIONS} & \textbf{Step} & \textbf{Descrizione} \\
\hline
Bug non in stato corretto & 2.a & Mostra ``Bug non pu\`o essere risolto in questo stato'' \\
\cline{2-3}
...
\cline{2-3}
 & 6.b & L'utente pu\`o proseguire senza allegati \\
\hline
\end{tabular}
...
\hline
Goal & L'amministratore vuole generare un report sull'attivit\`a del sistema per un mese specifico \\
\hline
...
\begin{itemize}
  ...
  \item \textbf{RNF-P2 --- Paginazione}: La lista dei bug \`e paginata (default 20 elementi per pagina) per evitare il caricamento di dataset molto grandi.
...
La prototipazione visuale di BugBoard26 \`e stata realizzata direttamente come applicazione React funzionante, con dati simulati (\emph{mock data}). Le schermate qui mostrate sono catture reali del prototipo interattivo, e non semplici bozzetti grafici. Questo approccio ha permesso di validare il flusso utente in modo concreto fin dalla fase di analisi.
...
\begin{itemize}
  \item \textbf{Priorit\`a}: \emph{Critical} (rosso), \emph{High} (arancione), \emph{Medium} (giallo), \emph{Low} (verde)
  \item \textbf{Stato}: \emph{Todo} (grigio chiaro), \emph{In Progress} (blu), \emph{Resolved} (verde), \emph{Archived} (grigio scuro)
  \item \textbf{Tipo}: \emph{Bug} (rosso), \emph{Feature} (viola), \emph{Enhancement} (azzurro)
...
\begin{itemize}
  ...
  \item \textbf{Accessi rapidi}: da qui si pu\`o navigare alla lista bug, al form di creazione e ai report
\end{itemize}
...
\caption{Lista Bug: tabella con filtri per tipo, stato, priorit\`a e ricerca testuale}
\end{figure}

\begin{itemize}
  \item \textbf{Scopo}: visualizzazione e filtraggio dell'elenco completo dei bug
  \item \textbf{Elementi}: barra filtri orizzontale (tipo, stato, priorit\`a), campo ricerca full-text, tabella bug con badge colorati, paginazione
...
\begin{itemize}
  ...
  \item \textbf{Elementi}: header con titolo, stato e priorit\`a; sezioni descrizione, commenti, allegati, storico; barra azioni laterale (modifica, assegna, risolvi, archivia, duplica)
...
\caption{Form Creazione Bug (UC01): campi titolo, tipo, priorit\`a, descrizione, label, allegati}
\end{figure}

\begin{itemize}
  \item \textbf{Caso d'uso}: UC01 --- Creazione nuovo bug
  \item \textbf{Elementi}: campo titolo, selettori tipo e priorit\`a, area di testo descrizione, selezione label multipla, upload allegati immagine, selezione assegnatario opzionale, pulsante ``Salva''
  \item \textbf{Validazione}: i campi obbligatori (titolo, tipo, priorit\`a) sono evidenziati se lasciati vuoti; il formato e la dimensione degli allegati sono verificati prima dell'invio
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Elementi}: selettore mese/anno, grafici (distribuzione stato, priorit\`a, trend temporale), tabella riassuntiva metriche, pulsante ``Esporta Excel''
...
\begin{itemize}
  ...
  \item \textbf{Elementi}: tabella bug in stato \emph{Archived} con filtri e ricerca; solo Admin pu\`o archiviare/ripristinare bug
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Navigazione mobile}: il menu hamburger con le stesse voci del desktop ha garantito piena parit\`a funzionale su dispositivi mobili
\end{itemize}
...
\hline
Vite & 6.x & Build tool moderno, significativamente pi\`u veloce di Webpack grazie a ESBuild e HMR nativo. \\
\hline
...
\begin{itemize}
  ...
  \item \textbf{Proxy in sviluppo}: Vite redirige \texttt{/api/*} a \texttt{localhost:8081}, evitando problemi CORS durante lo sviluppo
...
Il diagramma delle classi di design \`e organizzato secondo il pattern architetturale \textbf{layered architecture} con i seguenti package:

\begin{description}[style=nextline]
\item[\texttt{model}] Entit\`a JPA che mappano le tabelle del database: \texttt{User}, \texttt{Bug}, \texttt{Comment}, \texttt{History}, \texttt{Label} e le enumerazioni \texttt{Role}, \texttt{BugType}, \texttt{BugStatus}, \texttt{Priority}, \texttt{HistoryAction}.

\item[\texttt{repository}] Interfacce Spring Data JPA che estendono \texttt{JpaRepository<T, UUID>}: \texttt{UserRepository}, \texttt{BugRepository} (+ \texttt{JpaSpecificationExecutor}), \texttt{CommentRepository}, \texttt{LabelRepository}, \texttt{HistoryRepository}.

\item[\texttt{service}] Classi di business logic: \texttt{BugService} (gestione completa del ciclo di vita dei bug), \texttt{AuthService} (registrazione, login, generazione token).

\item[\texttt{dto}] Record Java per i dati di trasferimento: \texttt{BugCreateRequest}, \texttt{BugPatchRequest}, \texttt{AssignRequest}, \texttt{LabelSetRequest}, \texttt{LoginRequest}.

\item[\texttt{controller}] Controller REST: \texttt{BugController} (\texttt{/api/bugs}), \texttt{AuthController} (\texttt{/api/auth}), \texttt{AdminController} (\texttt{/api/admin}), \texttt{ExportController} (\texttt{/api/export}).
...
Il diagramma completo, generato tramite Mermaid, \`e riportato in Figura~\ref{fig:class_diagram}.
...
\begin{itemize}
  ...
  \item \textbf{Isolamento}: le dipendenze (repository JPA) sono sostituite da \emph{mock objects} iniettati con \texttt{@Mock} / \texttt{@InjectMocks}, garantendo che i test verifichino esclusivamente la logica di business senza accesso al database.
\end{itemize}
...
\textbf{Ingresso:} codice sorgente compilabile; dipendenze Maven risolte; ambiente Java 23 configurato.

\textbf{Uscita:} tutti i 9 test passano (\texttt{BUILD SUCCESS}) con 0 failures e 0 errors; copertura dei rami decisionali principali raggiunta.
...
\begin{itemize}
  ...
  \item \textbf{Velocit\`a}: nessun I/O di rete o disco; l'intera suite viene eseguita in meno di 2 secondi.
...
Tutti i 9 casi di test passano con successo, confermando il corretto funzionamento della logica di business per le tre funzionalit\`a testate.
...
\begin{itemize}
  \item \textbf{SonarLint 10.x} integrato in IntelliJ IDEA, con regole allineate al profilo \emph{Sonar way} per Java
  \item \textbf{Compilatore Java 23} (Amazon Corretto) con \texttt{-Xlint:all}
  \item \textbf{JUnit 5 + Mockito 5} per la verifica della copertura funzionale
  \item Revisione manuale del codice (\emph{code review})
\end{itemize}

\subsection{Dashboard di qualit\`a (stile SonarQube)}

La seguente tabella riassume le metriche principali, emulando la dashboard di un server SonarQube con il profilo \emph{Sonar way} per Java.
...
\emph{Rating SonarQube: A = ottimo, B = buono, C = accettabile, D = scarso, E = critico. Il Quality Gate ``Passed'' indica che il progetto soddisfa le soglie minime di qualit\`a.}
...
\hline
\texttt{model} & 10 & $\sim$550 & Entit\`a JPA + 5 enumerazioni \\
\hline
\texttt{service} & 5 & $\sim$610 & Business logic e specifiche \\
\hline
\texttt{controller} & 5 & $\sim$490 & Endpoint REST \\
\hline
\texttt{dto} & 8 & $\sim$95 & Record per request/response \\
\hline
\texttt{repository} & 5 & $\sim$61 & Interfacce Spring Data JPA \\
\hline
\texttt{security} & 2 & $\sim$145 & JWT service e filter \\
\hline
\texttt{config} & 2 & $\sim$90 & Security config e seed \\
\hline
\texttt{root} & 1 & $\sim$7 & Entry point \\
\hline
\end{tabular}
...
\subsubsection{Code Smells (5 issue --- Rating A)}
...
\subsubsection{Security Hotspot (1 issue --- Rating A)}

Il security hotspot rilevato riguarda la configurazione CORS che potrebbe essere troppo permissiva in ambiente di produzione. In sviluppo, la configurazione consente richieste da \texttt{localhost} per facilitare il debug.

\subsection{Duplicazioni}
Il tasso di duplicazione del codice \`e dell'1.6\%, ben al di sotto della soglia di allerta del 3\%. Le duplicazioni residue si trovano principalmente nei metodi di lookup utente nei servizi.

\subsection{Punti di forza}
\begin{itemize}
  \item Zero vulnerabilit\`a di sicurezza identificate
  \item Architettura pulita con separazione netta tra layer
  \item DTO immutabili implementati come record Java
  \item Test di unit\`a con copertura funzionale completa (9/9 passati)
  \item Technical debt contenuto ($\sim$1h)
\end{itemize}

\subsection{Riepilogo Quality Gate}
Il progetto supera il Quality Gate con \textbf{Rating A} su tutte le metriche principali:
\begin{itemize}
  \item Bugs: 0 (soglia: 0) \checkmark
  \item Vulnerabilit\`a: 0 (soglia: 0) \checkmark
  \item Security Hotspots: 1, reviewed \checkmark
  \item Code Smells: 5, Rating A \checkmark
  \item Duplicazioni: 1.6\% < 3\% \checkmark
  \item Test Success Rate: 100\% (9/9) \checkmark
...
\begin{itemize}
  \item \textbf{T4} --- P3 non ha individuato immediatamente il pulsante ``Assegna'' nella vista dettaglio del bug; ha cercato la funzione nel menu laterale.
  \item \textbf{T7} --- P5 ha confuso il pulsante ``Archivia'' con ``Elimina'' e ha esitato prima di confermare l'azione.
\end{itemize}
...
\vspace{0.3cm}
\noindent Le azioni AC-1 e AC-2 sono state implementate nella release corrente. Le restanti sono pianificate per iterazioni future.
