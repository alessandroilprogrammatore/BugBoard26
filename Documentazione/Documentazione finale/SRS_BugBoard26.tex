...
\item[\textbf{USER (Utente registrato)}] Ruolo standard. L'utente pu\`o creare bug, commentare, caricare allegati, visualizzare i bug assegnati a lui e gestire il proprio profilo.
...
\item[\textbf{Utente non registrato (Guest)}] Chiunque acceda all'applicazione senza credenziali. Pu\`o accedere alla pagina di login e alla pagina informativa del progetto. Non ha accesso ad alcuna funzionalit\`a interna.
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item Gestione assegnazioni, archiviazione e scadenze dei bug
  \item Accesso completo a tutti i bug del sistema
\end{itemize} \\
\hline
Sistema &
...
\begin{itemize}
  \item \textbf{Visualizzazione informazioni pubbliche}: Accesso alla pagina principale dell'applicazione con informazioni generali sul servizio di bug tracking e indicazioni su come richiedere un account.
...
\begin{itemize}
  ...
  \item \textbf{Esportazione dati}: Estrazione completa dei dati in formato CSV, Excel o PDF per analisi esterne e condivisione.
  ...
  \item \textbf{Gestione assegnazioni, archiviazione e scadenze}: Possibilit\`a di assegnare bug, archiviarli e impostare deadline opzionali per la risoluzione.
  \item \textbf{Accesso completo a tutti i bug}: Possibilit\`a di visualizzare e modificare qualsiasi bug del sistema indipendentemente dall'assegnazione.
\end{itemize}
...
\hline
Step 8 & & Prepara il documento nel formato di esportazione selezionato \\
\hline
...
\begin{itemize}
  \item \textbf{RNF-S1 --- Autenticazione}: Tutti gli endpoint (eccetto login) richiedono un token JWT valido. Il token ha una durata configurabile (default: 8 ore).
...
\caption{Report Mensile Admin (UC04): statistiche aggregate e dettaglio per utente con opzioni di export}
\end{figure}

\begin{itemize}
  \item \textbf{Caso d'uso}: UC04 --- Generazione report mensile (solo Admin)
  \item \textbf{Elementi}: selettore mese/anno, statistiche aggregate, tabella riassuntiva metriche, dettaglio per utente e opzioni di export
...
\caption{Export Dati: download dell'elenco bug in formato CSV, Excel o PDF}
\end{figure}

\begin{itemize}
  \item \textbf{Scopo}: consentire l'esportazione dei dati per analisi esterne
  \item \textbf{Elementi}: pulsanti ``Scarica CSV'', ``Scarica Excel'' e ``Scarica PDF''; i file vengono generati dal backend e scaricati direttamente
\end{itemize}
...
\hline
PDFBox & 3.0.2 & Generazione di documenti PDF per l'export dell'elenco bug. \\
\hline
\end{tabular}
...
\item[\texttt{service}] Classi di business logic: \texttt{BugService} (gestione completa del ciclo di vita dei bug), \texttt{AuthService} (login, logout, generazione token).
